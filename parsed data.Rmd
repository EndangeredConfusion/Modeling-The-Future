...
```{r}
df_short <- df_short %>% mutate(Wday = wday(df_short$Date)) # Add day of week column
df_short <- df_short %>% mutate(Month = month(df_short$Date)) # Add month column
df_short <- df_short %>% mutate(Year = year(df_short$Date)) # Add year column
...
for (area in community_areas) {
  df_short[sprintf('comm_area_%d', area)] <- df_short$`Community Area` == area
}
```
```{r}
commarea_model <- lm(crimes_by_temp ~ temps + comm_area_1, crimes_by_temp_frame)
print(summary(temp_model_commarea))
```
